...
public sealed class Literal : Expression
{
    ...
    internal Literal(TokenType tokenType, object? value, StringSegment raw) : base(Nodes.Literal)
    {
        TokenType = tokenType;
        Value = value;
        Raw = raw;
    ...
    public Literal(StringSegment value, StringSegment raw) : this(TokenType.StringLiteral, value, raw)
    {
    }

    public Literal(bool value, StringSegment raw) : this(TokenType.BooleanLiteral, value ? s_boxedTrue : s_boxedFalse, raw)
    {
    }

    public Literal(double value, StringSegment raw) : this(TokenType.NumericLiteral, value, raw)
    {
    }

    public Literal(BigInteger value, StringSegment raw) : this(TokenType.BigIntLiteral, value, raw)
    {
    }

    public Literal(StringSegment raw) : this(TokenType.NullLiteral, null, raw)
    {
    }

    public Literal(StringSegment pattern, StringSegment flags, object? value, StringSegment raw) : this(TokenType.RegularExpression, value, raw)
    {
    ...
    public StringSegment? StringValue { [MethodImpl(MethodImplOptions.AggressiveInlining)] get => TokenType == TokenType.StringLiteral ? (StringSegment) Value! : null; }
